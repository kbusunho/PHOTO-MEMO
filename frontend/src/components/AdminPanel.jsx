...
function AdminPanel({ currentUser, onClose }) { 
  ...
  const handleUserUpdated = (updatedUser) => {
    ...
    if (updatedUser._id === currentUser.id) {
      // AuthContext의 user를 업데이트 하려면 App 전체 리로드(window.location.reload())가 가장 간단
      toast.success('본인 정보가 수정되었습니다. 변경사항은 다음 로그인 시 반영됩니다.');
    }
  };

  const handleDeleteUser = async (userToDelete) => {
    if (userToDelete._id === currentUser.id) {
      toast.error("자기 자신은 삭제할 수 없습니다."); // 2. alert -> toast.error
      return;
    }
    ...
    if (window.confirm(`정말 '${userToDelete.email}' 사용자를 삭제하시겠습니까?\n이 사용자의 모든 맛집 기록도 함께 삭제됩니다.`)) {
      try {
        ...
        toast.success("사용자가 삭제되었습니다."); // 3. alert -> toast.success
      } catch (err) {
        toast.error(`삭제 실패: ${err.response?.data?.message || '서버 오류'}`); // 4. alert -> toast.error
      }
    }
  };
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-30 p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-5xl relative animate-fade-in-up max-h-[90vh] flex flex-col">
          ...
          {loading && <p className="text-gray-500 dark:text-gray-400 text-center">사용자 목록을 불러오는 중...</p>}
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-100 dark:bg-gray-700 sticky top-0">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">닉네임</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">권한</th>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {users.map((user) => (
                    <tr key={user._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{user.displayName || 'N/A'}</td>
